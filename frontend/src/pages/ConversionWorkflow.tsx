...
const ConversionWorkflow: React.FC<ConversionWorkflowProps> = ({ onBack }) => {
  ...
  const [businessClassResolved, setBusinessClassResolved] = useState(false);
  const [bcWarning, setBcWarning] = useState('');
  ...
  // Fetch available business classes on mount
  useEffect(() => {
    api.get('/schema/available-business-classes')
      .then(res => setAvailableBusinessClasses(res.data.business_classes || []))
      .catch(() => {});
  }, []);

  // When businessClass changes, check if it matches an available schema
  useEffect(() => {
    if (!businessClass) {
      setBusinessClassResolved(false);
      setBcWarning('');
      return;
    }
    const match = availableBusinessClasses.find(bc => bc.toLowerCase() === businessClass.toLowerCase());
    if (match) {
      setBusinessClassResolved(true);
      setBcWarning('');
      if (match !== businessClass) setBusinessClass(match); // normalize casing
    } else {
      setBusinessClassResolved(false);
      setBcWarning(`No schema found for "${businessClass}". Select from available schemas.`);
    }
  }, [businessClass, availableBusinessClasses]);
  ...
  useEffect(() => {
    const autoPopulateOnLoadStep = async () => {
      ...
      if (currentStep === 'load' && showInterfaceForm && jobId && mappingData.mapping) {
        // Compute RunGroup from filename (same logic as Step 2)
        const runGroupFromFile = file?.name
          ? file.name.replace(/\.[^/.]+$/, '').replace(/\s+/g, '_').slice(0, 30)
          : (mappingData.mapping?.['_auto_rungroup_']?.run_group_value || '');
        
        if (!interfaceParams.enterpriseGroup || interfaceParams.runGroup !== runGroupFromFile) {
          ...
          setInterfaceParams(prev => ({
            ...prev,
            runGroup: runGroupFromFile,
            enterpriseGroup: financeEnterpriseGroup || prev.enterpriseGroup
          }));
        }
      }
    };
  ...
  const handleFileUpload = async () => {
    ...
    try {
      ...
      if (response.data.detection) {
        ...
        // Only set business class from detection if user hasn't already resolved it
        if (!businessClassResolved) {
      ...
      // Use the resolved business class from Step 1 (user may have corrected it)
      await performAutomaticMapping(response.data.job_id, businessClass || detectedClass);
    } catch (error: any) {
  ...
  const handleStartValidation = async () => {
    ...
    try {
      ...
      const response = await api.post('/validation/start', {
        ...
        selected_rule_set_id: selectedRuleSetId,
        date_source_format: dateTransformEnabled ? dateSourceFormat : null
      });
  ...
  const proceedWithLoad = async () => {
    ...
    try {
      ...
      // Get the filename-based RunGroup from Step 2
      const runGroupOverride = file?.name
        ? file.name.replace(/\.[^/.]+$/, '').replace(/\s+/g, '_').slice(0, 30)
        : (mappingData.mapping?.['_auto_rungroup_']?.run_group_value || '');
      ...
      const response = await api.post('/load/start', {
        ...
        mapping: enabledMapping,
        chunk_size: 1000,
        run_group_override: runGroupOverride || undefined,
      ...
      {currentStep === 'upload' && (
        <div className={getAnimationClass()} style={{
          ...
          <div style={{ marginBottom: '20px' }}>
            ...
            <div style={{ position: 'relative', width: '400px' }}>
              <input
                ...
                style={{
                  ...
                  padding: '12px',
                  backgroundColor: theme.background.secondary,
                  border: `1px solid ${bcWarning ? theme.status.error : (businessClassResolved ? theme.status.success : theme.background.quaternary)}`,
                  borderRadius: '6px',
                  color: businessClassResolved ? theme.status.success : theme.text.primary,
                  fontSize: '14px',
                  fontWeight: '500',
                  boxSizing: 'border-box' as const
                }}
                placeholder="Auto-detected from filename or type to search"
              />
              {businessClassResolved && (
                <span style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', color: theme.status.success, fontSize: 16 }}>✓</span>
              )}
              {bcDropdownOpen && availableBusinessClasses.length > 0 && (
                <div style={{
            ...
            {bcWarning && (
              <div style={{ marginTop: 6, fontSize: 12, color: theme.status.error, display: 'flex', alignItems: 'center', gap: 6 }}>
                ⚠️ {bcWarning}
              </div>
            )}
          </div>
          ...
          <div style={{ marginBottom: '20px' }}>
            ...
            <input
              ...
              onChange={(e) => {
                ...
                // Detect business class from filename immediately
                if (selectedFile) {
                  const name = selectedFile.name.replace(/\.[^/.]+$/, '');
                  const detected = name.split('_')[0];
                  setBusinessClass(detected);
                } else {
                  setBusinessClass('');
                }
              }}
            ...
            <div style={{
              ...
              <div style={{ marginBottom: '12px' }}>
                ...
                <span style={{ 
                  ...
                  {businessClass || detectionResult.business_class}
                </span>
              </div>
          ...
          <button
            ...
            disabled={!file || uploading || !businessClassResolved}
            style={{
              padding: '12px 24px',
              backgroundColor: file && !uploading && businessClassResolved ? theme.primary.main : theme.interactive.disabled,
              color: file && !uploading && businessClassResolved ? '#ffffff' : theme.text.muted,
              border: 'none',
              borderRadius: '6px',
              cursor: file && !uploading && businessClassResolved ? 'pointer' : 'not-allowed',
              fontSize: '14px',
              fontWeight: '500'
            }}
          >
          ...
          {!validating && validationProgress?.status !== 'validated' && (
            <div style={{
              ...
              <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: dateTransformEnabled ? 12 : 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{ fontSize: 15 }}>🔄</span>
                    <div>
                      <div style={{ fontSize: 13, fontWeight: 600, color: theme.text.primary }}>Date Format Conversion</div>
                      <div style={{ fontSize: 11, color: theme.text.muted }}>Convert date fields to FSM format (YYYYMMDD) in-memory</div>
                    </div>
                  </div>
                  <div
                  ...
                  >
                    <div style={{
                      ...
                      left: dateTransformEnabled ? 20 : 2,
                ...
                {dateTransformEnabled && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    ...
                    <div style={{
                      ...
                      borderRadius: 6, fontSize: 13, fontWeight: 600, color: '#166534'
                    }}>
                      YYYYMMDD
                    </div>
                  </div>
                  ...
                  <div style={{
                    ...
                    <div style={{ flex: '1 1 calc(50% - 8px)', minWidth: '300px' }}>
                      <label style={{ fontSize: '12px', color: theme.text.primary, marginBottom: '4px', display: 'block', fontWeight: '500' }}>
                        RunGroup * (From filename)
                      </label>
                      <input
                        type="text"
                        value={file?.name ? file.name.replace(/\.[^/.]+$/, '').replace(/\s+/g, '_').slice(0, 30) : interfaceParams.runGroup || ''}
                        readOnly
